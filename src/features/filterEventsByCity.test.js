...
defineFeature(feature, (test) => {
  test('When user hasn´t searched for a city, show upcoming events from all cities.', ({
    ...
    given('user hasn’t searched for any city', () => {}) // no precondition, so need to enter anything here
  ...
  test('User can select a city from the suggested list.', ({
    ...
    when(
      ...
      async () => {
        const user = userEvent.setup()
        await user.click(suggestionListItems[0]) // this is not an empty array or object, but the list item
      }
    )
    ...
    and(
      ...
      async () => {
        ...
        // filtering only events in Berlin, citysearchInput.value should be 'Berlin, Germany'
